...
flags.DEFINE_string("save_path", './save/', "Directory to write the model and "
                    "training summaries.")
...
class Word2Vec(object):
  ...
  def forward(self, examples, labels):
    ...
    if experiment:
        ...
        with tf.variable_scope("NN", initializer=tf.uniform_unit_scaling_initializer(factor=1.15)):
            self.W_1 = tf.get_variable("W_1", [opts.emb_dim, opts.num_hidden_1])
            self.b_1 = tf.get_variable("b_1", [opts.num_hidden_1])
            self.W_2 = tf.get_variable("W_2", [opts.num_hidden_1, opts.num_hidden_2])
            self.b_2 = tf.get_variable("b_2", [opts.num_hidden_2])

        # Softmax weight: [vocab_size, emb_dim]. Transposed.
        sm_w_t = tf.Variable(
            tf.zeros([opts.vocab_size, opts.num_hidden_2]),
            name="sm_w_t")
    else:
        # Softmax weight: [vocab_size, emb_dim]. Transposed.
        sm_w_t = tf.Variable(
    ...
    if experiment:
        ...
        tmp = tf.nn.embedding_lookup(emb, examples)

        # Go through NN
        y_1 = tf.sigmoid(tf.matmul(tmp, self.W_1)+self.b_1)
  ...
  def optimize(self, loss):
    ...
    if experiment:
        ...
        train = self.optimizer.minimize(loss,
                               ...
                               gate_gradients=self.optimizer.GATE_NONE)
    else:
  ...
  def build_eval_graph(self):
    ...
    def getEmb(analogy):
        emb = tf.gather(self._emb, analogy)

        # Go through NN
        y_1 = tf.sigmoid(tf.matmul(emb, self.W_1)+self.b_1)
        y_2 = tf.sigmoid(tf.matmul(y_1, self.W_2)+self.b_2)

        # normalization
        ny_2 = tf.nn.l2_normalize(y_2, 1)

        return ny_2

    if experiment:
        a_emb = getEmb(analogy_a)
        b_emb = getEmb(analogy_b)
        c_emb = getEmb(analogy_c)
    else:
    ...
    if experiment:
        # Go through NN
        y_1 = tf.sigmoid(tf.matmul(self._emb, self.W_1)+self.b_1)
        y_2 = tf.sigmoid(tf.matmul(y_1, self.W_2)+self.b_2)
        # normalization
        ny_2 = tf.nn.l2_normalize(y_2, 1)

        dist = tf.matmul(target, ny_2, transpose_b=True)
    else:
    ...
    nearby_emb = tf.gather(ny_2, nearby_word)
    nearby_dist = tf.matmul(nearby_emb, ny_2, transpose_b=True)
  ...
  def eval(self):
    ...
    print("Eval %4d/%d accuracy = %4.1f%%" % (correct, total,
                                              correct * 100.0 / total))
    f_acc=open('acc_log.txt','a')
    f_acc.write(str(correct * 100.0 / total)+'\n')
    f_acc.close()
