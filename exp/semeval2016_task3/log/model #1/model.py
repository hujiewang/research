...
class RankNet:
    def __init__(self, is_training=True, config=defaultConfig):
        ...
        self._grads = tf.gradients(self._cost, self._tvars)
        optimizer = tf.train.AdamOptimizer(self.lr)
        self._train_op = optimizer.apply_gradients(zip(self._grads, self._tvars))
        '''
    ...
    def generateTmpTrain(self, cur_ori_q_idx):
        ...
        for i in range(_len):
            for j in range(_len):
                ...
                label=1
                if attr_i['RELQ_RELEVANCE2ORGQ']!=attr_j['RELQ_RELEVANCE2ORGQ']:
