...
class Model(object):
    def __init__(self):
        ...
        def _rnn(inputs):
            with tf.variable_scope("GRU_RNN") as scope:
                ...
                for time, input_ in enumerate(inputs):
                    # input_=tf.nn.dropout(input_, self.dropout_input)
        ...
        state_0_body_normalized = tf.nn.l2_normalize(state_1_, 1)

        dist_title_ = tf.mul(state_0_title_normalized, state_1_title_normalized)
        dist_body_ = tf.mul(state_0_title_normalized, state_1_title_normalized)
